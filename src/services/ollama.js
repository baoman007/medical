...
// 医疗专用模型
const MEDICAL_MODEL = 'medical-v1'

// 通用模型（备用）
const FALLBACK_MODEL = 'qwen2.5'

/**
 * 调用本地Ollama模型进行对话
 */
export async function chatWithLocalModel(messages, options = {}) {
  ...
  try {
    ...
    if (model === MEDICAL_MODEL) {
      console.log('尝试使用备用模型:', FALLBACK_MODEL)
 ...
 * 分析用户症状并生成结构化建议
 */
export async function analyzeSymptom(userInput) {
 ...
 * 检查是否为紧急情况
 */
export function checkEmergency(userInput) {
